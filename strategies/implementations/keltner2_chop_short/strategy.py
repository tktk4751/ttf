...
from .signal_generator import Keltner2ChopShortSignalGenerator


class Keltner2ChopShortStrategy(BaseStrategy):
    """
    2つのケルトナーチャネル+チョピネスインデックスを使用したショート戦略
    ...
    def __init__(
        ...
        """初期化"""
        super().__init__("Keltner2ChopShortStrategy")
        ...
        # シグナル生成器の初期化
        self.signal_generator = Keltner2ChopShortSignalGenerator(
            entry_period=entry_period,
            entry_atr_period=entry_atr_period,
            entry_multiplier=entry_multiplier,
            exit_period=exit_period,
    ...
    def create_optimization_params(cls, trial: optuna.Trial) -> Dict[str, Any]:
        """最適化パラメータの作成"""
        return {
            'entry_period': trial.suggest_int('entry_period', 10, 50),
            'entry_atr_period': trial.suggest_int('entry_atr_period', 5, 25),
            'entry_multiplier': trial.suggest_float('entry_multiplier', 1.0, 3.0),
            'exit_period': trial.suggest_int('exit_period', 10, 50),
            'exit_atr_period': trial.suggest_int('exit_atr_period', 5, 25),
            'exit_multiplier': trial.suggest_float('exit_multiplier', 1.0, 3.0),
            'chop_period': trial.suggest_int('chop_period', 10, 30),
            'chop_threshold': trial.suggest_float('chop_threshold', 30.0, 70.0)
        }
