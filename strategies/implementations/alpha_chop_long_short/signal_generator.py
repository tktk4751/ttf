...
def calculate_entry_signals(alpha: np.ndarray, filter_signal: np.ndarray) -> np.ndarray:
    ...
    # ロングエントリー: Alphaチャネルの買いシグナル + チョピネスがトレンド
    long_condition = (alpha == 1) & (filter_signal == 1)
    # ショートエントリー: Alphaチャネルの売りシグナル + チョピネスがトレンド
    short_condition = (alpha == -1) & (filter_signal == 1)
...
class AlphaChopLongShortSignalGenerator(BaseSignalGenerator):
    """
    Alphaチャネル+チョピネスフィルターのシグナル生成クラス（ロング・ショート両用・高速化版）
    ...
    エグジット条件:
    - ロング：Alphaチャネルの売りシグナル
    - ショート：Alphaチャネルの買いシグナル
    """
    ...
    def __init__(
        ...
        """初期化"""
        super().__init__("AlphaChopLongShortSignalGenerator")
