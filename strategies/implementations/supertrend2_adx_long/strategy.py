...
from .signal_generator import Supertrend2ADXLongSignalGenerator


class Supertrend2ADXLongStrategy(BaseStrategy):
    """
    スーパートレンド+ADXフィルター戦略（買い専用）
    ...
    def __init__(
        ...
        """
        super().__init__("Supertrend2ADXLong")
        ...
        # シグナル生成器の初期化
        self.signal_generator = Supertrend2ADXLongSignalGenerator(
            entry_period=entry_period,
            entry_multiplier=entry_multiplier,
            exit_period=exit_period,
            exit_multiplier=exit_multiplier,
