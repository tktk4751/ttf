...
def calculate_entry_signals(alpha: np.ndarray, filter_signal: np.ndarray) -> np.ndarray:
    ...
    # ショートエントリー: Alphaチャネルの売りシグナル + チョピネスが非トレンド
...
class AlphaChopShortSignalGenerator(BaseSignalGenerator):
    """
    Alphaチャネル+チョピネスフィルターのシグナル生成クラス（ショート専用・高速化版）
    ...
    def __init__(
        ...
        """初期化"""
        super().__init__("AlphaChopShortSignalGenerator")
