...
def calculate_entry_signals(tfc: np.ndarray, filter_signal: np.ndarray) -> np.ndarray:
    ...
    # ショートエントリー: TFCが-1 + チョピネスがトレンド
    short_condition = (tfc == -1) & (filter_signal == 1)
...
class TFCChopSingleShortSignalGenerator(BaseSignalGenerator):
    """
    TFC+チョピネスフィルターのシグナル生成クラス（ショート専用・高速化版）
    ...
    def __init__(
        ...
        """初期化"""
        super().__init__("TFCChopSingleShortSignalGenerator")
