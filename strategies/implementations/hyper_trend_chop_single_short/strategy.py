...
from .signal_generator import HyperTrendChopSingleShortSignalGenerator


class HyperTrendChopSingleShortStrategy(BaseStrategy):
    """
    HyperTrend+チョピネスフィルター戦略（ショート専用）
    ...
    def __init__(
        ...
        """
        super().__init__("HyperTrendChopSingleShort")
        ...
        # シグナル生成器の初期化
        self.signal_generator = HyperTrendChopSingleShortSignalGenerator(
            er_period=er_period,
            max_percentile_length=max_percentile_length,
            min_percentile_length=min_percentile_length,
            max_atr_period=max_atr_period,
