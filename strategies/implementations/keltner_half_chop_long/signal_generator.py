...
from signals.implementations.keltner.half_breakout_entry import KeltnerHalfBreakoutEntrySignal
...
class KeltnerHalfChopLongSignalGenerator(BaseSignalGenerator):
    """
    ケルトナーチャネル+チョピネスフィルターのシグナル生成クラス（買い専用・高速化版）
    
    エントリー条件:
    - ケルトナーチャネルのアッパーブレイクアウトで買いシグナル
    - チョピネスインデックスがトレンド相場を示している
    
    エグジット条件:
    - ケルトナーチャネルのロワーハーフラインを下回る
    """
    ...
    def __init__(
        ...
        """初期化"""
        super().__init__("KeltnerHalfChopLongSignalGenerator")
        ...
        self.exit_signal = KeltnerHalfBreakoutEntrySignal(
            period=keltner_period,
            atr_period=atr_period,
            multiplier=multiplier
        )
    ...
    def calculate_signals(self, data: Union[pd.DataFrame, np.ndarray]) -> None:
        ...
        if self._signals is None or current_len != self._data_len:
            ...
            half_signals = self.exit_signal.generate(df).astype(np.int8)
            
            # エントリーシグナルの計算（ベクトル化）
            self._signals = calculate_entry_signals(keltner_signals, filter_signals)
            
            # エグジット用のシグナルを事前計算
            self._half_signals = half_signals
    ...
    def get_exit_signals(self, data: Union[pd.DataFrame, np.ndarray], position: int, index: int = -1) -> bool:
        ...
        # キャッシュされたシグナルを使用
        return bool(self._half_signals[index] == -1) 