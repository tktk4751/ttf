...
class KeltnerDualMultiplierShortSignalGenerator(BaseSignalGenerator):
    """
    ケルトナーチャネル+チョピネスフィルターのシグナル生成クラス（売買別ATR乗数・売り専用・高速化版）
    ...
    def __init__(
        ...
        """初期化"""
        super().__init__("KeltnerDualMultiplierShortSignalGenerator")
    ...
    def calculate_signals(self, data: Union[pd.DataFrame, np.ndarray]) -> None:
        ...
        if self._signals is None or current_len != self._data_len:
            ...
            if result is None:
                self._upper = np.zeros(current_len)
            else:
                # ATRの取得
                atr = self.signal._keltner.atr.calculate(df)
                # アッパーバンドの計算
                self._upper = result.middle + (self.upper_multiplier * atr)
    ...
    def get_exit_signals(self, data: Union[pd.DataFrame, np.ndarray], position: int, index: int = -1) -> bool:
        ...
        # 売りポジションのエグジット判定
        return close[index] > self._upper[index] 