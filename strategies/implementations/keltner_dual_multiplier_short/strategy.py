...
from .signal_generator import KeltnerDualMultiplierShortSignalGenerator


class KeltnerDualMultiplierShortStrategy(BaseStrategy):
    """
    ケルトナーチャネル+チョピネスフィルター戦略（売買別ATR乗数・売り専用）
    
    エントリー条件:
    - ロワーブレイクアウトで売りシグナル（lower_multiplierを使用）
    ...
    def __init__(
        ...
        """
        super().__init__("KeltnerDualMultiplierShort")
        ...
        # シグナル生成器の初期化
        self.signal_generator = KeltnerDualMultiplierShortSignalGenerator(
            ema_period=ema_period,
            atr_period=atr_period,
            upper_multiplier=upper_multiplier,
            lower_multiplier=lower_multiplier,
    ...
    def create_optimization_params(cls, trial: optuna.Trial) -> Dict[str, Any]:
        ...
        """
        params = {
            'ema_period': trial.suggest_int('ema_period', 5, 100),
