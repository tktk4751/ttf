...
from .signal_generator import DonchianATRChopShortSignalGenerator


class DonchianATRChopShortStrategy(BaseStrategy):
    """
    ドンチャンATR+チョピネスフィルター戦略（売り専用）
    
    エントリー条件:
    - ドンチャンATRのロワーブレイクアウトで売りシグナル
    ...
    def __init__(
        ...
        """
        super().__init__("DonchianATRChopShort")
        ...
        # シグナル生成器の初期化
        self.signal_generator = DonchianATRChopShortSignalGenerator(
            period=period,
            atr_period=atr_period,
            upper_multiplier=upper_multiplier,
            lower_multiplier=lower_multiplier,
    ...
    def create_optimization_params(cls, trial: optuna.Trial) -> Dict[str, Any]:
        ...
        params = {
            ...
            'upper_multiplier': trial.suggest_float('upper_multiplier', 0.0, 3.0, step=0.1),  # ロング版と逆にする
            'lower_multiplier': trial.suggest_float('lower_multiplier', 0.0, 4.0, step=0.1),  # ロング版と逆にする
            'chop_period': 55,
            'chop_threshold': 50.0,
        }
        return params
