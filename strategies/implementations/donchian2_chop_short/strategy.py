...
from .signal_generator import Donchian2ChopShortSignalGenerator


class Donchian2ChopShortStrategy(BaseStrategy):
    """
    2つのドンチャン+チョピネスフィルター戦略（売り専用）
    
    エントリー条件:
    - エントリー用ドンチャンチャネルのロワーブレイクアウトで売りシグナル
    - チョピネスインデックスがトレンド相場を示している
    
    エグジット条件:
    - エグジット用ドンチャンチャネルの買いシグナル
    """
    ...
    def __init__(
        ...
        """
        super().__init__("Donchian2ChopShort")
        ...
        # シグナル生成器の初期化
        self.signal_generator = Donchian2ChopShortSignalGenerator(
            entry_period=entry_period,
            exit_period=exit_period,
            chop_period=chop_period,
            chop_threshold=chop_threshold,
