...
@jit(nopython=True)
def calculate_entry_signals(supertrend: np.ndarray) -> np.ndarray:
...
class Supertrend2ShortSignalGenerator(BaseSignalGenerator):
    """
    2つのスーパートレンドのシグナル生成クラス（売り専用・高速化版）
    ...
    def __init__(
        ...
        """初期化"""
        super().__init__("Supertrend2ShortSignalGenerator")
