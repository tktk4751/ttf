...
from .signal_generator import Supertrend2ShortSignalGenerator


class Supertrend2ShortStrategy(BaseStrategy):
    """
    2つのスーパートレンド戦略（売り専用）
    ...
    def __init__(
        ...
        """
        super().__init__("Supertrend2Short")
        ...
        # シグナル生成器の初期化
        self.signal_generator = Supertrend2ShortSignalGenerator(
            entry_period=entry_period,
            entry_multiplier=entry_multiplier,
            exit_period=exit_period,
            exit_multiplier=exit_multiplier,
    ...
    def create_optimization_params(cls, trial: optuna.Trial) -> Dict[str, Any]:
        ...
        params = {
            ...
            'entry_multiplier': trial.suggest_float('entry_multiplier', 1.0, 10.0, step=0.1),
            'exit_period': trial.suggest_int('exit_period', 3, 120,),
            'exit_multiplier': trial.suggest_float('exit_multiplier', 1.0, 10.0, step=0.1),
        }
        return params
