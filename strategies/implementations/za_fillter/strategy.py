...
from .signal_generator import ZACTISignalGenerator


class ZACTIStrategy(BaseStrategy):
    """
    Zアダプティブチャネルとサイクルトレンドインデックスを組み合わせたストラテジー
    
    特徴:
    - サイクル効率比（CER）に基づく動的パラメータ最適化
    - ZAdaptiveChannelによる高精度なエントリーポイント検出
    - サイクルトレンドインデックスによるトレンド/レンジ市場の判別
    ...
    def __init__(
        ...
        Args:
            ...
            # サイクルトレンドインデックスフィルターのパラメータ
            cti_detector_type: サイクルトレンドインデックス用ドミナントサイクル検出器タイプ
            cti_cycle_part: サイクルトレンドインデックス用サイクル部分
            cti_max_cycle: サイクルトレンドインデックス用最大サイクル期間
            cti_min_cycle: サイクルトレンドインデックス用最小サイクル期間
            cti_max_output: サイクルトレンドインデックス用最大出力値
            cti_min_output: サイクルトレンドインデックス用最小出力値
            cti_src_type: サイクルトレンドインデックス用価格ソース
            cti_lp_period: サイクルトレンドインデックス用ローパスフィルター期間
            cti_hp_period: サイクルトレンドインデックス用ハイパスフィルター期間
            smooth_er: 効率比にスムージングを適用するかどうか
            er_alma_period: 効率比スムージングの期間
            er_alma_offset: 効率比スムージングのオフセット
            er_alma_sigma: 効率比スムージングのシグマ
        ...
        # シグナル生成器の初期化
        self.signal_generator = ZACTISignalGenerator(
            # 基本パラメータ
            band_lookback=band_lookback,
            src_type=src_type,
    ...
    def create_optimization_params(cls, trial: optuna.Trial) -> Dict[str, Any]:
        ...
        params = {
            ...
            # サイクルトレンドインデックスフィルターのパラメータ
            'cti_detector_type': trial.suggest_categorical('cti_detector_type', 
                                                        ['dudi', 'hody', 'phac', 'dudi_e', 'hody_e', 'phac_e']),
            'cti_cycle_part': trial.suggest_float('cti_cycle_part', 0.2, 0.9, step=0.1),
            'cti_max_cycle': trial.suggest_int('cti_max_cycle', 34, 144),
            'cti_min_cycle': trial.suggest_int('cti_min_cycle', 3, 13),
            'max_threshold': trial.suggest_float('max_threshold', 0.6, 0.9, step=0.05),
            'min_threshold': trial.suggest_float('min_threshold', 0.3, 0.6, step=0.05),
        }
        return params
    ...
    def convert_params_to_strategy_format(cls, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        strategy_params = {
            ...
            'use_kalman_filter': params['use_kalman_filter'],
            ...
            # サイクルトレンドインデックスフィルターのパラメータ
            'cti_detector_type': params['cti_detector_type'],
            'cti_cycle_part': params['cti_cycle_part'],
            'cti_max_cycle': params['cti_max_cycle'],
            'cti_min_cycle': params['cti_min_cycle'],
            'cti_max_output': params['cti_max_output'],
            'cti_min_output': params['cti_min_output'],
            'cti_src_type': params['cti_src_type'],
            'cti_lp_period': params['cti_lp_period'],
            'cti_hp_period': params['cti_hp_period'],
            'smooth_er': params['smooth_er'],
            'er_alma_period': params['er_alma_period'],
            'er_alma_offset': params['er_alma_offset'],
            'er_alma_sigma': params['er_alma_sigma'],
            'smooth_chop': params['smooth_chop'],
            'chop_alma_period': params['chop_alma_period'],
            'chop_alma_offset': params['chop_alma_offset'],
            'chop_alma_sigma': params['chop_alma_sigma'],
