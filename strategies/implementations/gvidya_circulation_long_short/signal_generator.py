...
from signals.implementations.gvidya.direction import GVIDYACirculationSignal
...
def calculate_entry_signals(gvidya_signals: np.ndarray, chop_signals: np.ndarray) -> np.ndarray:
    ...
    signals = np.zeros_like(gvidya_signals, dtype=np.int8)
    
    # ロングエントリー: GVIDYAが上昇 + チョピネスがレンジ
    long_condition = (gvidya_signals == 1) & (chop_signals == 1)
    # ショートエントリー: GVIDYAが下降 + チョピネスがレンジ
    short_condition = (gvidya_signals == -1) & (chop_signals == 1)
...
class GVIDYACirculationLongShortSignalGenerator(BaseSignalGenerator):
    """
    GVIDYAサーキュレーションのシグナル生成クラス（ロング・ショート両対応・高速化版）
    
    エントリー条件:
    [ロング]
    - GVIDYAサーキュレーションが上昇相場を示している
    - チョピネスフィルターがレンジ相場を示している
    
    [ショート]
    - GVIDYAサーキュレーションが下降相場を示している
    - チョピネスフィルターがレンジ相場を示している
    ...
    def __init__(
        ...
        """初期化"""
        super().__init__("GVIDYACirculationLongShortSignalGenerator")
        
        # シグナル生成器の初期化
        self.gvidya_signal = GVIDYACirculationSignal(
            params={
