...
from signals.implementations.rsi.exit import RSIExitSignal, RSIExit2Signal
...
class DonchianChopShortSignalGenerator(BaseSignalGenerator):
    """
    ドンチャンチャネル+チョピネスフィルターのシグナル生成クラス（売り専用・高速化版）
    
    エントリー条件:
    - ドンチャンチャネルのブレイクアウトで売りシグナル
    - チョピネスインデックスがトレンド相場を示している
    
    エグジット条件:
    - ドンチャンチャネルの買いシグナル
    - RSIのエグジットシグナル（RSI <= 14からRSI >= 14）
    - RSIが70以上
    """
    ...
    def __init__(
        ...
        rsi_period: int = 14,
        rsi_exit_threshold: float = 14.0,
        rsi_exit2_threshold: float = 80.0
    ):
        """初期化"""
        super().__init__("DonchianChopShortSignalGenerator")
        
        # シグナル生成器の初期化
        self.entry_signal = DonchianBreakoutEntrySignal(period=donchian_period)
        ...
        self.exit_signal = RSIExitSignal(
            period=rsi_period,
            solid={
                'rsi_long_exit_solid': 86,  # 使用しない
                'rsi_short_exit_solid': rsi_exit_threshold
            }
        )
        self.exit2_signal = RSIExit2Signal(
            period=rsi_period,
            solid={
                'rsi_long_exit_solid': 30,  # 使用しない
                'rsi_short_exit_solid': rsi_exit2_threshold
            }
        )
