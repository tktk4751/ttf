...
from .signal_generator import AlphaTrendSignalGenerator


class AlphaTrendStrategy(BaseStrategy):
    """
    アルファトレンド+アダプティブチョピネスフィルター戦略
    ...
    def __init__(
        ...
        Args:
            period: KAMAの効率比の計算期間（デフォルト: 13）
            max_kama_slow: KAMAの遅い移動平均の最大期間（デフォルト: 89）
            min_kama_slow: KAMAの遅い移動平均の最小期間（デフォルト: 30）
            max_kama_fast: KAMAの速い移動平均の最大期間（デフォルト: 15）
            ...
            chop_mfast: アダプティブチョピネスの最小期間（デフォルト: 2）
            chop_mslow: アダプティブチョピネスの最大期間（デフォルト: 30）
            max_threshold: チョピネスのしきい値の最大値（デフォルト: 0.6）
            min_threshold: チョピネスのしきい値の最小値（デフォルト: 0.4）
        """
        ...
        # シグナル生成器の初期化
        self.signal_generator = AlphaTrendSignalGenerator(
            period=period,
            max_kama_slow=max_kama_slow,
            min_kama_slow=min_kama_slow,
            max_kama_fast=max_kama_fast,
    ...
    def create_optimization_params(cls, trial: optuna.Trial) -> Dict[str, Any]:
        ...
        """
        params = {
            'period': trial.suggest_int('period', 3, 200),
            'max_kama_slow': 100,
            'min_kama_slow': 25,
            'max_kama_fast': 20,
            'min_kama_fast': 2,
