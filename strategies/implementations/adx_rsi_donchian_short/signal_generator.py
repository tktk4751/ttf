...
def calculate_entry_signals(adx_signals: np.ndarray, rsi_signals: np.ndarray, donchian_signals: np.ndarray) -> np.ndarray:
    ...
    for i in range(len(signals)):
        # ADXフィルターがトレンド相場、RSIが売りシグナル、ドンチャンシグナルが0の場合
        if adx_signals[i] == 1 and rsi_signals[i] == -1 and donchian_signals[i] == 0:
            signals[i] = -1
    
    return signals.astype(np.int8)

class ADXRSIDonchianShortSignalGenerator(BaseSignalGenerator):
    """
    ADX+RSI+ドンチャンのシグナル生成クラス（売り専用・高速化版）
    ...
    def __init__(
        ...
        """初期化"""
        super().__init__("ADXRSIDonchianShortSignalGenerator")
