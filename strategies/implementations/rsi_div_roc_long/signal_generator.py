...
def calculate_entry_signals(div_signals: np.ndarray, roc_signals: np.ndarray, lookback: int) -> np.ndarray:
    ...
    for i in range(lookback, len(div_signals)):
        ...
        if div_signals[i] == 1:
            # 現在のROCシグナルが買いシグナルか確認
            if roc_signals[i] == 1:
                signals[i] = 1
        elif div_signals[i-lookback:i].max() == 1 and roc_signals[i] == 1:
            signals[i] = 1
    
    return signals.astype(np.int8)

class RSIDivROCLongSignalGenerator(BaseSignalGenerator):
    """
    RSIダイバージェンス+ROCのシグナル生成クラス（買い専用・高速化版）
    
    エントリー条件:
    - RSIダイバージェンスが買いシグナルを出してから13期間以内
    - ROCが買いシグナル
    ...
    def __init__(
        ...
        div_lookback: int = 30,
        roc_period: int = 21,
        entry_rsi_period: int = 14,  # エントリー用RSI期間
        exit_rsi_period: int = 14,   # エグジット用RSI期間
        entry_lookback: int = 13,
    ):
        """初期化"""
        super().__init__("RSIDivROCLongSignalGenerator")
        
        # シグナル生成器の初期化
        self._entry_rsi = RSI(entry_rsi_period)  # エントリー用RSI
        self._exit_rsi = RSI(exit_rsi_period)    # エグジット用RSI
        self.div_signal = DivergenceSignal(lookback=div_lookback)
        self.roc_signal = ROCEntrySignal(period=roc_period)
        self.exit_signal = RSIExitSignal(period=exit_rsi_period)
    ...
    def calculate_signals(self, data: Union[pd.DataFrame, np.ndarray]) -> None:
        ...
        if self._signals is None or current_len != self._data_len:
            ...
            # RSI値の計算（エントリー用とエグジット用）
            entry_rsi_values = self._entry_rsi.calculate(data)
            
            # 各シグナルの計算（一度に実行）
            div_signals = self.div_signal.generate(df, entry_rsi_values)  # エントリー用RSIを使用
