...
def calculate_entry_signals(supertrend_signals: np.ndarray, chop_signals: np.ndarray, roc_signals: np.ndarray) -> np.ndarray:
    ...
    for i in range(len(signals)):
        # スーパートレンドが売り、CHOPフィルターがトレンド相場、ROCが売りシグナルの場合
        if supertrend_signals[i] == -1 and chop_signals[i] == 1 and roc_signals[i] == -1:
            signals[i] = -1
    
    return signals.astype(np.int8)

class SupertrendChopROCShortSignalGenerator(BaseSignalGenerator):
    """
    スーパートレンド+CHOP+ROCのシグナル生成クラス（売り専用・高速化版）
    ...
    def __init__(
        ...
        """初期化"""
        super().__init__("SupertrendChopROCShortSignalGenerator")
