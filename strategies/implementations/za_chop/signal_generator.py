...
from signals.implementations.cycle_chop.filter import CycleChoppinessFilterSignal


@njit(fastmath=True, parallel=True)
def generate_combined_signals_numba(
    z_adaptive_signals: np.ndarray,
    cycle_chop_signals: np.ndarray
) -> np.ndarray:
    """
    Zアダプティブチャネルシグナルとサイクルチョピネスシグナルを組み合わせた複合シグナルを生成する
    
    Args:
        z_adaptive_signals: Zアダプティブチャネルシグナルの配列
        cycle_chop_signals: サイクルチョピネスシグナルの配列
    ...
    for i in prange(length):
        if np.isnan(z_adaptive_signals[i]) or np.isnan(cycle_chop_signals[i]):
            signals[i] = 0
        elif z_adaptive_signals[i] == 1 and cycle_chop_signals[i] == -1:
            signals[i] = 1  # ロングエントリー（トレンド相場で買い）
        elif z_adaptive_signals[i] == -1 and cycle_chop_signals[i] == -1:
            signals[i] = -1  # ショートエントリー（トレンド相場で売り）
    
    return signals


class ZACHOPSignalGenerator(BaseSignalGenerator):
    """
    Zアダプティブチャネルとサイクルチョピネスフィルターを組み合わせたシグナル生成クラス
    
    エントリー条件:
    - ロング: Zアダプティブチャネルのシグナルが1で、サイクルチョピネスのシグナルが-1（トレンド相場で買い）
    - ショート: Zアダプティブチャネルのシグナルが-1で、サイクルチョピネスのシグナルが-1（トレンド相場で売り）
    ...
    def __init__(
        ...
        """初期化"""
        super().__init__("ZACHOPSignalGenerator")
        ...
        # サイクルチョピネスフィルターの初期化
        self.cycle_chop_filter = CycleChoppinessFilterSignal(
        ...
        try:
            ...
            if self._signals is None or current_len != self._data_len:
                ...
                try:
                    ...
                    self._cycle_chop_signals = np.zeros(current_len, dtype=np.int8)
            ...
            if data is not None:
                ...
                self._cycle_chop_signals = np.zeros(len(data), dtype=np.int8)
    ...
    def get_cycle_chop_filter_values(self, data: Union[pd.DataFrame, np.ndarray] = None) -> np.ndarray:
        """
        サイクルチョピネスフィルター値を取得
        ...
        Returns:
            np.ndarray: サイクルチョピネスフィルター値
        """
        try:
            if data is not None:
                self.calculate_signals(data)
                
            return self.cycle_chop_filter.get_filter_values()
        except Exception as e:
            self.logger.error(f"サイクルチョピネスフィルター値取得中にエラー: {str(e)}")
            return np.array([])
    
    def get_cycle_chop_threshold(self, data: Union[pd.DataFrame, np.ndarray] = None) -> np.ndarray:
        """
        サイクルチョピネスフィルターの動的しきい値を取得
        ...
        try:
            ...
            return self.cycle_chop_filter.get_threshold_values()
        except Exception as e:
    ...
    def get_er_values(self, data: Union[pd.DataFrame, np.ndarray] = None) -> np.ndarray:
        ...
        try:
            ...
            return self.cycle_chop_filter.get_er_values()
        except Exception as e:
            self.logger.error(f"サイクル効率比取得中にエラー: {str(e)}")
            return np.array([]) 