...
from .signal_generator import SupertrendDualChopLongSignalGenerator

class SupertrendDualChopLongStrategy(BaseStrategy):
    """デュアルスーパートレンド+チョピネスフィルターの買い専用戦略"""
    ...
    def __init__(
        ...
        """
        super().__init__("SupertrendDualChopLong")
        ...
        # シグナル生成器の初期化
        self._signal_generator = SupertrendDualChopLongSignalGenerator(
            period=period,
            fast_multiplier=fast_multiplier,
            slow_multiplier=slow_multiplier,
            chop_period=chop_period,
    ...
    def get_stop_price(self, data: Union[pd.DataFrame, np.ndarray], position: int, index: int = -1) -> float:
        ...
        Returns:
            float: ストップロス価格（スロウスーパートレンドの下限値）
        """
        if position != 1:  # ロングポジションのみ対応
            raise ValueError("This strategy only supports long positions")
