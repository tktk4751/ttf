...
from signals.implementations.hyper_donchian_frama.entry import HyperDonchianFRAMACrossoverEntrySignal


class FilterType(Enum):
    """HyperドンチャンFRAMAストラテジー用のフィルタータイプ"""
    NONE = "none"
    HYPER_ER = "hyper_er"
...
class HyperDonchianFRAMASignalGenerator:
    """HyperドンチャンFRAMAシグナルジェネレーター"""
    ...
    def __init__(self, params: Dict[str, Any]):
        ...
        hyper_donchian_period_min = entry_params.get('hyper_donchian_period_min', 55.0)
        hyper_donchian_period_max = entry_params.get('hyper_donchian_period_max', 250.0)
        
        self.entry_signal = HyperDonchianFRAMACrossoverEntrySignal(
            hyper_donchian_period=entry_params.get('hyper_donchian_period', 20),
            hyper_donchian_src_type=entry_params.get('hyper_donchian_src_type', 'hlc3'),
...
if __name__ == "__main__":
    ...
    print("=== HyperドンチャンFRAMAシグナルジェネレーターのテスト ===")
    ...
    signal_generator = HyperDonchianFRAMASignalGenerator(signal_params)
    ...
    consensus_generator = HyperDonchianFRAMASignalGenerator(consensus_params)
    ...
    print("\n従来のドンチャンFRAMAとの比較...")
    try:
        from ..donchian_frama.signal_generator import DonchianFRAMASignalGenerator
        ...
        traditional_generator = DonchianFRAMASignalGenerator(traditional_params)
        ...
        print("  従来のドンチャンFRAMAシグナルジェネレーターが見つかりませんでした")
