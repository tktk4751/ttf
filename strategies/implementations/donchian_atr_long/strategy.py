...
from .signal_generator import DonchianATRLongSignalGenerator


class DonchianATRLongStrategy(BaseStrategy):
    """
    2つのドンチャンATR戦略（買い専用）
    
    エントリー条件:
    - エントリー用ドンチャンATRのアッパーブレイクアウトで買いシグナル
    
    エグジット条件:
    - エグジット用ドンチャンATRの売りシグナル
    """
    ...
    def __init__(
        ...
        """
        super().__init__("DonchianATRLong")
        ...
        # シグナル生成器の初期化
        self.signal_generator = DonchianATRLongSignalGenerator(
            entry_period=entry_period,
            entry_atr_period=entry_atr_period,
            entry_multiplier=entry_multiplier,
            exit_period=exit_period,
