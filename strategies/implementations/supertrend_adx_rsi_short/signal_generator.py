...
def calculate_entry_signals(supertrend_signals: np.ndarray, adx_signals: np.ndarray, rsi_signals: np.ndarray) -> np.ndarray:
    ...
    for i in range(len(signals)):
        # スーパートレンドが売り、ADXフィルターが有効、RSIが売りシグナルの場合
        if supertrend_signals[i] == -1 and adx_signals[i] == 1 and rsi_signals[i] == -1:
            signals[i] = -1
    
    return signals.astype(np.int8)

class SupertrendADXRSIShortSignalGenerator(BaseSignalGenerator):
    """
    スーパートレンド+ADX+RSIのシグナル生成クラス（売り専用・高速化版）
    ...
    def __init__(
        ...
        """初期化"""
        super().__init__("SupertrendADXRSIShortSignalGenerator")
