...
def calculate_entry_signals(chop_signals: np.ndarray, rsi_signals: np.ndarray, donchian_signals: np.ndarray) -> np.ndarray:
    ...
    for i in range(len(signals)):
        # CHOPフィルターがレンジ相場、RSIが買いシグナル、ドンチャンシグナルが0の場合
        if chop_signals[i] == -1 and rsi_signals[i] == 1 and donchian_signals[i] == 0:
            signals[i] = 1
    
    return signals.astype(np.int8)

class ChopRSIDonchianLongSignalGenerator(BaseSignalGenerator):
    """
    CHOP+RSI+ドンチャンのシグナル生成クラス（買い専用・高速化版）
    ...
    def __init__(
        ...
        """初期化"""
        super().__init__("ChopRSIDonchianLongSignalGenerator")
