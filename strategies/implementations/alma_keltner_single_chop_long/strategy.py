...
from .signal_generator import ALMAKeltnerSingleChopLongSignalGenerator


class ALMAKeltnerSingleChopLongStrategy(BaseStrategy):
    """
    ALMAケルトナーチャネル+チョピネスフィルター戦略（単一チャネル・買い専用）
    
    エントリー条件:
    - ALMAケルトナーチャネルのアッパーブレイクアウトで買いシグナル
    - チョピネスインデックスがトレンド相場を示している
    
    エグジット条件:
    - ALMAケルトナーチャネルの売りシグナル
    """
    ...
    def __init__(
        ...
        """
        super().__init__("ALMAKeltnerSingleChopLong")
        ...
        # シグナル生成器の初期化
        self.signal_generator = ALMAKeltnerSingleChopLongSignalGenerator(
            alma_period=alma_period,
            alma_offset=alma_offset,
            alma_sigma=alma_sigma,
            atr_period=atr_period,
    ...
    def create_optimization_params(cls, trial: optuna.Trial) -> Dict[str, Any]:
        ...
        """
        params = {
            'alma_period': trial.suggest_int('alma_period', 5, 300, step=5),
            'alma_offset': 0.85,
            'alma_sigma': 6,
