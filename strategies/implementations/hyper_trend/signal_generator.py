...
def calculate_entry_signals(direction: np.ndarray, filter_signal: np.ndarray) -> np.ndarray:
    ...
    # ロングエントリー: HyperTrendが上昇トレンド（1）+ ガーディアンエンジェルがトレンド相場（1）
    long_condition = (direction == 1) & (filter_signal == 1)
    
    # ショートエントリー: HyperTrendが下降トレンド（-1）+ ガーディアンエンジェルがトレンド相場（1）
...
class HyperTrendSignalGenerator(BaseSignalGenerator):
    """
    HyperTrend+ガーディアンエンジェルフィルターのシグナル生成クラス（両方向・高速化版）
    ...
    def __init__(
        ...
        """初期化"""
        super().__init__("HyperTrendSignalGenerator")
