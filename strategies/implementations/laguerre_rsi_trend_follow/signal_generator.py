...
def laguerre_rsi_trend_follow_exit_conditions(lrsi_signals: np.ndarray, position: int, index: int) -> bool:
    """
    Laguerre RSI トレンドフォロー エグジット条件をチェック（Numba最適化版）
...
class LaguerreRSITrendFollowSignalGenerator(BaseSignalGenerator):
    """
    Laguerre RSI トレンドフォロー シグナル生成クラス
    ...
    エントリー条件:
    - ロング: RSI > buy_band (0.8)
    - ショート: RSI < sell_band (0.2)
    ...
    def __init__(
        ...
        self.lrsi_entry_signal = LaguerreRSITrendFollowEntrySignal(
            gamma=gamma,
            src_type=src_type,
            buy_band=buy_band,
            sell_band=sell_band,
    ...
    def get_exit_signals(self, data: Union[pd.DataFrame, np.ndarray], position: int, index: int = -1) -> bool:
        ...
        # Numba最適化された関数を使用
        return laguerre_rsi_trend_follow_exit_conditions(self._lrsi_signals, position, index)
    ...
    def get_advanced_metrics(self, data: Union[pd.DataFrame, np.ndarray] = None) -> Dict[str, np.ndarray]:
        ...
        try:
            ...
            metrics = {
                ...
                # パラメータ情報
                'strategy_type': 'laguerre_rsi_trend_follow',
