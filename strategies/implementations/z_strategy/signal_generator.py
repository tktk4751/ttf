...
def calculate_entry_signals(
    ...
    breakout_signal : np.ndarray
        ZChannelBreakoutEntrySignalの配列（1=ロング、-1=ショート）
    filter_signal : np.ndarray
    ...
    # ロングエントリー: ZTrendDirectionSignalが1(上昇トレンド) + ZChannelBreakoutEntrySignalが1(ロングブレイクアウト) + ZTrendFilterSignalが1(トレンド相場)
    long_condition = (direction_signal == 1) & (breakout_signal == 1) & (filter_signal == 1)
    
    # ショートエントリー: ZTrendDirectionSignalが-1(下降トレンド) + ZChannelBreakoutEntrySignalが-1(ショートブレイクアウト) + ZTrendFilterSignalが1(トレンド相場)
    short_condition = (direction_signal == -1) & (breakout_signal == -1) & (filter_signal == 1)
...
class ZSignalGenerator(BaseSignalGenerator):
    """
    ZTrendDirectionSignal + ZChannelBreakoutEntrySignal + ZTrendFilterSignalのシグナル生成クラス（両方向・高速化版）
    ...
    def __init__(
        ...
        """初期化"""
        super().__init__("ZSignalGenerator")
        ...
        # ZChannelBreakoutEntrySignalの初期化
        self.breakout_signal = ZChannelBreakoutEntrySignal(
            cycle_detector_type=cycle_detector_type,
            lp_period=lp_period,
            hp_period=hp_period,
            cycle_part=cycle_part,
        ...
        try:
            ...
            if self._signals is None or current_len != self._data_len:
                ...
                try:
                    ...
                    breakout_values = self.breakout_signal.generate(df)
                    ...
                    self._breakout_values = breakout_values
                    ...
                    self._breakout_values = np.zeros(current_len, dtype=np.int8)
                    self._filter_values = np.zeros(current_len, dtype=np.int8)
            ...
            if data is not None:
                ...
                self._breakout_values = np.zeros(len(data), dtype=np.int8)
                self._filter_values = np.zeros(len(data), dtype=np.int8)
    ...
    def get_exit_signals(self, data: Union[pd.DataFrame, np.ndarray], position: int, index: int = -1) -> bool:
        ...
        if position == 1:  # ロングポジション
            # ZChannelBreakoutEntrySignalが-1になったらロング決済
            return bool(self._breakout_values[index] == -1)
        elif position == -1:  # ショートポジション
            # ZChannelBreakoutEntrySignalが1になったらショート決済
            return bool(self._breakout_values[index] == 1)
        return False
    
    def get_direction_signals(self, data: Union[pd.DataFrame, np.ndarray] = None) -> np.ndarray:
        """
        ZTrendDirectionSignalの値を取得
    ...
    def get_breakout_signals(self, data: Union[pd.DataFrame, np.ndarray] = None) -> np.ndarray:
        """
        ZChannelBreakoutEntrySignalの値を取得
    ...
    def get_filter_signals(self, data: Union[pd.DataFrame, np.ndarray] = None) -> np.ndarray:
        """
        ZTrendFilterSignalの値を取得
        
        Args:
            data: オプションの価格データ。指定された場合は計算を実行します。
            
        Returns:
            np.ndarray: フィルターシグナル値
        """
        try:
            if data is not None:
                self.calculate_signals(data)
    ...
    def get_band_values(self, data: Union[pd.DataFrame, np.ndarray] = None) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
        ...
        try:
            ...
            return self.breakout_signal.get_band_values()
        except Exception as e:
