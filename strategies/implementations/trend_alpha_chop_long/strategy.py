...
from .signal_generator import TrendAlphaChopLongSignalGenerator


class TrendAlphaChopLongStrategy(BaseStrategy):
    """
    TrendAlpha+チョピネスフィルター戦略（ロング専用）
    
    エントリー条件:
    - TrendAlphaのブレイクアウトで買いシグナル（ERに基づいて動的に調整）
    ...
    def __init__(
        ...
        """
        super().__init__("TrendAlphaChopLong")
        ...
        # シグナル生成器の初期化
        self.signal_generator = TrendAlphaChopLongSignalGenerator(
            kama_period=kama_period,
            kama_fast=kama_fast,
            kama_slow=kama_slow,
            max_atr_period=max_atr_period,
