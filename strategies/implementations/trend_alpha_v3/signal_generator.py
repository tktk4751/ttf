...
def calculate_entry_signals(trend_alpha: np.ndarray, filter_signal: np.ndarray) -> np.ndarray:
    ...
    # ロングエントリー: TrendAlphaの買いシグナル + アダプティブチョピネスがトレンド相場
    long_condition = (trend_alpha == 1) & (filter_signal == 1)
    
    # ショートエントリー: TrendAlphaの売りシグナル + アダプティブチョピネスがトレンド相場
...
class TrendAlphaV3SignalGenerator(BaseSignalGenerator):
    """
    TrendAlpha+アダプティブチョピネスフィルターのシグナル生成クラス（両方向・高速化版）
    ...
    def __init__(
        ...
        """初期化"""
        super().__init__("TrendAlphaV3SignalGenerator")
        ...
        self._params = {
            ...
            'chop_min_threshold': min_threshold
        }
