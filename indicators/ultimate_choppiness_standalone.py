...
# -*- coding: utf-8 -*-
"""
🚀 Ultimate Choppiness Index by John Ehlers - Standalone Version
循環インポートを回避した独立版
...
class UltimateChoppinessIndex:
    """
    🚀 Ultimate Choppiness Index - Standalone Version
    ...
    def __init__(self, 
        ...
        smoothing_period : int
            最終スムージング期間
        """
        self.name = f"UltimateChop({base_period})"
...
# 従来のChoppinessIndex（簡略版）
class SimpleChoppinessIndex:
    """従来のチョピネスインデックス（比較用）"""
    
    def __init__(self, period: int = 14):
        self.period = period
        self.name = f"CHOP({period})"
        self._values = None
    ...
    def calculate(self, data: Union[pd.DataFrame, np.ndarray]) -> np.ndarray:
        ...
        for i in range(self.period, length):
            ...
            if price_range > 0 and tr_sum > 0:
                chop[i] = 100.0 * np.log10(tr_sum / price_range) / np.log10(self.period)
                chop[i] = max(0.0, min(100.0, chop[i])) / 100.0  # 0-1に正規化
