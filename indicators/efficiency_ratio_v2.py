...
try:
    ...
    class PriceSource:
        @staticmethod
        def calculate_source(data, src_type, ukf_params=None):
            if isinstance(data, pd.DataFrame):
                ...
                elif src_type == 'ukf_hlc3': return ((data['high'] + data['low'] + data['close']) / 3).values  # Fallback
                else: return data['close'].values # Default to close
            else:
                return data[:, 3] if data.ndim > 1 and data.shape[1] > 3 else data
        @staticmethod
        def get_available_sources():
            return {'close': 'Close', 'hlc3': 'HLC3', 'ukf_hlc3': 'UKF HLC3'}
    class UltimateSmoother:
        def __init__(self, period=10, src_type='close', ukf_params=None):
            self.period = period
        def calculate(self, data):
            ...
            return UltimateSmootherResult(values=np.full(len(data), np.nan), coefficients=np.full(len(data), np.nan))
...
class ER_V2_Result(NamedTuple):
    """ER_V2計算結果"""
    values: np.ndarray
    smoothed_values: np.ndarray      # UltimateSmoother適用後の値
    trend_signals: np.ndarray        # 1=up, -1=down, 0=range
    current_trend: str               # 'up', 'down', 'range'
    current_trend_value: int         # 1, -1, 0
    dynamic_periods: np.ndarray      # 動的期間（動的期間モード時のみ）


@njit(fastmath=True, cache=True)
def calculate_efficiency_ratio_v2(change: np.ndarray, volatility: np.ndarray) -> np.ndarray:
    """
    効率比（Efficiency Ratio）を計算する（V2版・高速化）
...
@njit(fastmath=True, cache=True)
def calculate_efficiency_ratio_for_period_v2(prices: np.ndarray, period: int) -> np.ndarray:
    """
    指定された期間の効率比（ER）を計算する（V2版・高速化）
    
    Args:
        prices: 価格の配列（UKF適応済みソース）
        period: 計算期間
...
@njit(fastmath=True, cache=True)
def calculate_dynamic_er_v2_numba(
    prices: np.ndarray,
    period_array: np.ndarray,
    max_period: int
) -> np.ndarray:
    """
    動的期間でEfficiency Ratioを計算する（V2版・Numba JIT）
    
    Args:
        prices: 価格の配列（UKF適応済み）
...
def calculate_trend_signals_v2_with_range(values: np.ndarray, slope_index: int, range_threshold: float = 0.005) -> np.ndarray:
    """
    トレンド信号を計算する（V2版・range状態対応）(Numba JIT)
...
@jit(nopython=True, cache=True)
def calculate_current_trend_v2_with_range(trend_signals: np.ndarray) -> tuple:
    """
    現在のトレンド状態を計算する（V2版・range対応）(Numba JIT)
...
class ER_V2(Indicator):
    """
    効率比（Efficiency Ratio）V2インジケーター
    
    既存のERインジケーターをアップグレードした高性能版：
    - UKF_HLC3プライスソース（カルマンフィルター適応済み価格）を使用
    - UltimateSmoother による高品質な平滑化
    - 動的期間（ドミナントサイクル）または固定期間での計算に対応
    - 統計的閾値を使用したrange状態判定
    
    特徴:
    - カルマンフィルターによるノイズ除去済み価格での計算
    - John EhlersのUltimateSmoother による優れた平滑化
    - 適応的なトレンド判定
    - 高精度なレンジ相場検出
    ...
    def __init__(self, 
        ...
        Args:
            ...
            use_ultimate_smoother: UltimateSmootherを使用するか
            ...
            slope_index: トレンド判定期間 (1以上、デフォルト: 3)
            range_threshold: range判定の基本閾値（デフォルト: 0.005 = 0.5%）
            lp_period: ローパスフィルター期間（動的期間モード用、デフォルト: 5）
            hp_period: ハイパスフィルター期間（動的期間モード用、デフォルト: 120）
        """
        
        dynamic_str = f"_dynamic({detector_type})" if use_dynamic_period else ""
        smooth_str = f"_UltimateSmoother({smoother_period})" if use_ultimate_smoother else ""
        super().__init__(f"ER_V2(p={period},src={src_type}{dynamic_str}{smooth_str},slope={slope_index},range_th={range_threshold:.3f})")
        ...
        if self.use_ultimate_smoother:
            self.ultimate_smoother = UltimateSmoother(
                ...
                src_type='close',  # ER値は単一の値なので'close'を使用
                ukf_params=None    # ER値自体はUKFの必要なし
            )
        ...
        self._cache = {}
        self._result: Optional[ER_V2_Result] = None
    ...
    def _get_data_hash(self, data: Union[pd.DataFrame, np.ndarray]) -> str:
        ...
        # パラメータ文字列の作成
        ukf_str = str(self.ukf_params) if self.ukf_params else "None"
        if self.use_dynamic_period:
            param_str = (f"p={self.period}_src={self.src_type}_ukf={ukf_str}_smooth={self.use_ultimate_smoother}_{self.smoother_period}_"
                        f"dynamic={self.detector_type}_{self.max_output}_{self.min_output}_slope={self.slope_index}_range_th={self.range_threshold:.3f}")
        else:
            param_str = (f"p={self.period}_src={self.src_type}_ukf={ukf_str}_smooth={self.use_ultimate_smoother}_{self.smoother_period}_"
    ...
    def calculate(self, data: Union[pd.DataFrame, np.ndarray]) -> ER_V2_Result:
        """
        効率比V2を計算する
        ...
        try:
            ...
            if data_hash in self._cache and self._result is not None:
                ...
                if len(self._result.values) == current_data_len:
                    return ER_V2_Result(
                        ...
                        smoothed_values=self._result.smoothed_values.copy(),
            ...
            # PriceSourceを使ってUKF適応済み価格データを取得
            prices = PriceSource.calculate_source(data, self.src_type, self.ukf_params)
            ...
            if self.use_dynamic_period:
                ...
                if self.dc_detector is None:
                    ...
                    error_result = ER_V2_Result(
                        values=np.full(current_data_len, np.nan),
                        smoothed_values=np.full(current_data_len, np.nan),
                ...
                dynamic_periods = period_array.copy()
                ...
                # 動的ERの計算
                er_values = calculate_dynamic_er_v2_numba(prices, period_array, max_period_value)
            else:
                # 固定期間モード
                ...
                # 固定期間でのER計算
                er_values = calculate_efficiency_ratio_for_period_v2(prices, self.period)

            # 値を0-1の範囲に正規化（0以下は0、1以上は1にクリップ）
            er_values = np.clip(er_values, 0.0, 1.0)

            # UltimateSmoother による平滑化
            smoothed_values = er_values.copy()
            if self.use_ultimate_smoother and self.ultimate_smoother is not None:
                try:
                    # ER値をDataFrameに変換してUltimateSmootherに渡す
                    er_df = pd.DataFrame({'close': er_values})
                    smoother_result = self.ultimate_smoother.calculate(er_df)
                    ...
                    self.logger.warning(f"UltimateSmoother計算中にエラー: {e}. 元のER値を使用します。")
                    smoothed_values = er_values.copy()

            # トレンド判定（平滑化された値を使用）
            trend_signals = calculate_trend_signals_v2_with_range(smoothed_values, self.slope_index, self.range_threshold)
            trend_index, trend_value = calculate_current_trend_v2_with_range(trend_signals)
            ...
            return ER_V2_Result(
                ...
                smoothed_values=result.smoothed_values.copy(),
            ...
            self.logger.error(f"ER_V2 '{self.name}' 計算中に予期せぬエラー: {error_msg}\n{stack_trace}")
    ...
    def get_smoothed_values(self) -> Optional[np.ndarray]:
        """平滑化されたER値を取得する"""
        if self._result is not None:
            return self._result.smoothed_values.copy()
        return None
    ...
    def reset(self) -> None:
        ...
        if self.ultimate_smoother and hasattr(self.ultimate_smoother, 'reset'):
