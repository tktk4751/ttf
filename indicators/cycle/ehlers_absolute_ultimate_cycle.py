...
class EhlersAbsoluteUltimateCycle(EhlersDominantCycle):
    ...
    # 許可されるソースタイプのリスト
    SRC_TYPES = ['close', 'hlc3', 'hl2', 'ohlc4','ukf_hlc3','ukf_close','ukf','us_hlc3','us_close','oc2']
    ...
    def __init__(
        ...
        max_output: int = 60,
        min_output: int = 5,
        period_range: Tuple[int, int] = (5, 60),
        src_type: str = 'oc2',
        ...
        Args:
            ...
            period_range: サイクル期間の範囲（デフォルト: (5, 120)）
    ...
    def calculate(self, data: Union[pd.DataFrame, np.ndarray]) -> np.ndarray:
        ...
        try:
            ...
            try:
                ...
                # エラー時はフォールバック処理
                self.logger.warning(f"Numba関数でエラー: {str(e)}。フォールバック処理を実行します。")
                
                # 簡単なフォールバック実装
                n = len(price)
                dom_cycle = np.full(n, (self.max_output + self.min_output) / 2)
                raw_period = np.full(n, (self.period_range[0] + self.period_range[1]) / 2)
                confidences = np.full(n, 0.5)
                
                # 簡単な移動平均ベースのサイクル推定
                if n >= 10:
                    for i in range(10, n):
                        # 過去10期間の価格変動を見てサイクルを推定
                        recent_prices = price[i-10:i]
                        volatility = np.std(recent_prices)
                        if volatility > 0:
                            # ボラティリティに基づいてサイクル期間を調整
                            cycle_estimate = 20 + (volatility * 10)
                            cycle_estimate = max(self.min_output, min(self.max_output, cycle_estimate))
                            dom_cycle[i] = cycle_estimate * self.cycle_part
                            raw_period[i] = cycle_estimate
