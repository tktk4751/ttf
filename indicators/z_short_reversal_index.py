...
@dataclass
class ZShortReversalIndexResult:
    """Zショートリバーサルインデックスの計算結果"""
    values: np.ndarray          # Zショートリバーサルインデックスの値（0-1の範囲で正規化）
    zadx: np.ndarray            # ZADX値（0-1の範囲）
    inverted_zadx: np.ndarray   # 反転ZADX値（0-1の範囲）
    zrsx: np.ndarray            # ZRSX値（0-100の範囲）
    er: np.ndarray              # サイクル効率比（CER）
    combined_rms: np.ndarray    # 反転ZADX、ZRSX、ERの二乗平均平方根（RMS）
...
def calculate_dynamic_threshold(
    ...
    for i in range(length):
        ...
        # ERが高いほど（トレンドが強いほど）しきい値は低く
        # ERが低いほど（レンジ相場ほど）しきい値は高く
        threshold[i] = max_threshold - er_abs * (max_threshold - min_threshold)
    
    return threshold


@njit(fastmath=True)
def calculate_z_short_reversal_index(
    zadx: np.ndarray,  # 反転ZADX値（0-1の範囲）
    zrsx: np.ndarray,  # ZRSX値（0-1の範囲に正規化済み）
    er: np.ndarray, 
    ...
    """
    Zショートリバーサルインデックスを計算する
    
    Args:
        zadx: 反転したZADX値の配列（0-1の範囲）
        zrsx: ZRSX値の配列（0-1の範囲に正規化済み）
        er: 効率比の配列
...
class ZShortReversalIndex(Indicator):
    """
    Zショートリバーサルインデックス（ZShortReversalIndex）インジケーター
    
    反転ZADXとZRSXを様々な方法で組み合わせた高度なショートリバーサル検出インデックスです。
    ZADXの値を1から引いて反転させることで、ショートエントリーのリバーサル検出に最適化されています。
    
    特徴:
    - 反転ZADXを使用したトレンド強度の検出
    - ZRSXを使用した過買い/過売りの検出
    ...
    def __init__(
        ...
        """
        indicator_name = f"ZShortReversalIndex({combination_method}, {zadx_weight}, {zrsx_weight}, {cycle_detector_type})"
    ...
    def calculate(self, data: Union[pd.DataFrame, np.ndarray]) -> ZShortReversalIndexResult:
        """
        Zショートリバーサルインデックスを計算する
        ...
        Returns:
            ZShortReversalIndexResult: 計算結果を含むオブジェクト
        """
        try:
            ...
            # ZADX値を反転（1 - ZADX）
            inverted_zadx_values = 1.0 - zadx_values
            ...
            # Zショートリバーサルインデックスを計算（反転ZADX値を使用）
            combined_values, dynamic_threshold = calculate_z_short_reversal_index(
                inverted_zadx_values,  # 反転したZADX値を使用
            ...
            # 結果を保存
            self._result = ZShortReversalIndexResult(
                values=combined_values,
                zadx=zadx_values,  # 元のZADX値も保存
                inverted_zadx=inverted_zadx_values,  # 反転したZADX値を保存
            ...
            self.logger.error(f"ZShortReversalIndex計算中にエラー: {error_msg}\n{stack_trace}")
            
            # エラー時に空の結果を返す
            return ZShortReversalIndexResult(
                values=np.array([]),
                zadx=np.array([]),
                inverted_zadx=np.array([]),
                zrsx=np.array([]),
                er=np.array([]),
    ...
    def get_inverted_zadx(self) -> np.ndarray:
        """
        反転ZADX値を取得する（1 - ZADX）
        ...
        if self._result is None:
            return np.array([])
        return self._result.inverted_zadx
