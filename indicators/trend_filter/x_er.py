...
class XERResult:
    """X_ERの計算結果"""
    values: np.ndarray               # X_ER値（0-1の範囲、高い値=効率的トレンド）
    raw_er: np.ndarray              # 生のER値（カルマンフィルター適用前）
    filtered_er: np.ndarray         # カルマンフィルター適用後のER値
...
def calculate_x_er_numba(
    ...
    x_er = np.full(length, np.nan, dtype=np.float64)
    ...
    for i in range(period - 1, length):
        ...
        if valid_count >= current_period // 2:  # 有効なデータが半分以上の場合
            ...
            # X_ERは平均化されたER値そのものを使用（0-1の範囲は既に保証されている）
            x_er[i] = avg_er
...
def calculate_midline_and_signal_er(
    ...
    for i in range(midline_period - 1, length):
        # 期間内の最高値と最安値を計算
        period_data = x_er[i - midline_period + 1:i + 1]
...
class XER(Indicator, PercentileAnalysisMixin):
    """
    X_ER（X-Efficiency Ratio）インジケーター
    
    Efficiency Ratioをベースとした改良版トレンド効率性指標：
    
    特徴:
    - ソース価格→カルマンフィルター→ER計算→スムーサーの順序で計算
    - 0-1の値範囲で効率性を表現
    - 高い値=効率的なトレンド相場、低い値=非効率的なレンジ相場
    - 100期間ミッドラインによるトレンド判定機能
    - オプションで統合カルマンフィルター、統合スムーサー対応
    - 動的期間適応機能
    
    計算手順:
    1. ソース価格データを取得
    2. カルマンフィルターを適用（オプション）
    3. Efficiency Ratioを計算
    4. X_ERとして期間平均化
    5. スムーサーで平滑化（オプション）
    6. ミッドライン計算とトレンド判定
    """
    ...
    def __init__(
        ...
        Args:
            period: X_ER計算期間（デフォルト: 14）
            midline_period: ミッドライン計算期間（デフォルト: 100）
            er_period: ER期間（デフォルト: 5）
            er_src_type: ERソースタイプ（デフォルト: 'hlc3'）
        ...
        """
        indicator_name = f"X_ER({period}, midline={midline_period}, ER={er_period}({er_src_type})"
        ...
        # 内部でEfficiency Ratioを計算するため、外部依存なし
            ...
            if not UNIFIED_KALMAN_AVAILABLE:
                ...
                try:
                    self.kalman_filter = UnifiedKalman(
                        ...
                        src_type=self.er_src_type,  # ERと同じソースタイプを使用
    ...
    def _get_data_hash(self, data: Union[pd.DataFrame, np.ndarray]) -> str:
        ...
        try:
            ...
            param_str = (f"{self.period}_{self.midline_period}_{self.er_period}_"
                        f"{self.er_src_type}_{self.use_smoothing}_{self.smoother_type}_"
    ...
    def calculate(self, data: Union[pd.DataFrame, np.ndarray]) -> XERResult:
        """
        X_ERを計算する
        ...
        try:
            ...
            if self.use_kalman_filter and self.kalman_filter is not None:
                try:
                    ...
                    if isinstance(data, pd.DataFrame):
                        ...
                        if self.er_src_type not in kalman_input.columns:
                            kalman_input[self.er_src_type] = source_prices
                    else:
                        ...
                        if data.shape[1] >= 4:  # OHLC
                            ...
                            # 最小限のDataFrame
                            kalman_input = pd.DataFrame({self.er_src_type: source_prices})
            ...
            # 4. X_ERを計算
            x_er_values = calculate_x_er_numba(
                raw_er, self.period, dynamic_periods
            )
            
            # 5. 平滑化（オプション）
            smoothed_er = np.full_like(x_er_values, np.nan)
            if self.use_smoothing and self.smoother is not None:
                try:
                    # X_ER値をDataFrame形式に変換
                    er_df = pd.DataFrame({'close': x_er_values})
                    ...
                    smoothed_er = x_er_values.copy()
            else:
                smoothed_er = x_er_values.copy()
            
            # 最終的なX_ER値（平滑化が有効な場合は平滑化値、そうでなければ生の値）
            final_x_er = smoothed_er if self.use_smoothing else x_er_values
            
            # 6. ミッドラインとトレンド信号を計算
            midline, trend_signal = calculate_midline_and_signal_er(
                final_x_er, self.midline_period
            )
            ...
            # 結果の作成
            result = XERResult(
                values=final_x_er.copy(),
                raw_er=raw_er.copy(),
                filtered_er=x_er_values.copy(),
            ...
            self.logger.error(f"X_ER計算中にエラー: {error_msg}\\n{stack_trace}")
    ...
    def get_indicator_info(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'description': 'Efficiency Ratioベースの改良効率性指標（0-1範囲、高値=効率的トレンド、カルマンフィルター・動的期間・パーセンタイル分析対応）'
        }
    
    def _get_latest_result(self) -> Optional[XERResult]:
        """最新の結果を取得"""
        if not self._result_cache:
            return None
...
if __name__ == "__main__":
    ...
    result_kalman = x_er_kalman.calculate(df)
    
    valid_count_kalman = np.sum(~np.isnan(result_kalman.values))
    mean_x_er_kalman = np.nanmean(result_kalman.values)
    
    print(f"  有効値数: {valid_count_kalman}/{len(df)}")
    print(f"  平均X_ER（カルマン+平滑化）: {mean_x_er_kalman:.4f}")
    
    # 比較統計
    if valid_count > 0 and valid_count_kalman > 0:
        correlation = np.corrcoef(
            result.values[~np.isnan(result.values)][-min(valid_count, valid_count_kalman):],
            result_kalman.values[~np.isnan(result_kalman.values)][-min(valid_count, valid_count_kalman):]
        )[0, 1]
