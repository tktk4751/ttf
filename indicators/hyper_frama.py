...
@dataclass
class HyperFRAMAResult:
    """ハイパーFRAMAの計算結果"""
    frama_values: np.ndarray           # 通常のFRAMA値
    half_frama_values: np.ndarray      # アルファ調整のFRAMA値
    fractal_dimension: np.ndarray      # フラクタル次元
    alpha: np.ndarray                  # 通常のアルファ値
    half_alpha: np.ndarray             # 調整後のアルファ値
...
@njit(fastmath=True, cache=True)
def calculate_hyper_frama_core(price: np.ndarray, high: np.ndarray, low: np.ndarray, n: int, fc: int, sc: int, alpha_multiplier: float = 0.5) -> tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    """
    ハイパーFRAMA（通常のFRAMAとアルファ調整FRAMAを計算する
    ...
    Returns:
        ...
        通常FRAMA値、アルファ調整FRAMA値、フラクタル次元、アルファ値、調整アルファ値
    """
    length = len(price)
    
    # 結果配列を初期化
    frama = np.zeros(length, dtype=np.float64)
    half_frama = np.zeros(length, dtype=np.float64)
    dimension = np.zeros(length, dtype=np.float64)
    alpha = np.zeros(length, dtype=np.float64)
    half_alpha = np.zeros(length, dtype=np.float64)
    ...
    for i in range(min_period, length):
        if np.isnan(price[i]):
            ...
            half_frama[i] = half_frama[i-1] if i > 0 else np.nan
            continue
        ...
        # アルファ調整値の計算（調整係数を適用）
        adjusted_alpha_val = final_alpha * alpha_multiplier
        half_alpha[i] = adjusted_alpha_val
        ...
        if i == min_period:
            ...
            half_frama[i] = (1.0 - adjusted_alpha_val) * half_frama[i-1] + adjusted_alpha_val * price[i]
    
    return frama, half_frama, dimension, alpha, half_alpha


@njit(fastmath=True, cache=True)
def calculate_hyper_frama_indicator_adaptation_core(
    price: np.ndarray, 
    high: np.ndarray, 
    low: np.ndarray, 
    n: int, 
    ...
    """
    インジケーター動的適応ハイパーFRAMA計算（HyperERまたはHyperADX対応）
    ...
    Args:
        ...
        alpha_multiplier: アルファ調整係数（固定値）
    
    Returns:
        FRAMA値、アルファ調整FRAMA値、フラクタル次元、アルファ値、調整アルファ値のタプル
    """
    length = len(price)
    ...
    for i in range(length):
        ...
        # アルファ調整値の計算（固定alpha_multiplierを使用）
        adjusted_alpha_val = final_alpha * alpha_multiplier
        half_alpha[i] = adjusted_alpha_val
...
@njit(fastmath=True, cache=True)
def calculate_hyper_frama_dynamic_core(
    price: np.ndarray, 
    high: np.ndarray, 
    low: np.ndarray, 
    n: int, 
...
class HyperFRAMA(Indicator):
    """
    ハイパーFRAMA - 通常のFRAMAとアルファ調整FRAMAを同時計算するFractal Adaptive Moving Average
    By John Ehlers (改良版)
    
    フラクタル適応移動平均：
    - 価格データのフラクタル次元を計算
    - フラクタル次元に基づいて適応的にアルファ値を調整
    - 通常のFRAMAとアルファ調整FRAMAを同時に計算（MAMA/FAMAのコンセプト）
    - トレンド時は高速、レンジ時は低速で反応
    
    特徴:
    - 通常FRAMA: D=1時は高速移動平均（α=1）、D=2時は低速移動平均（α=0.01）
    - 調整FRAMA: 通常FRAMAのアルファに調整係数を掛けてより滑らかまたは応答性を調整
    - アルファ調整係数により柔軟な設定が可能
    - 複数のプライスソースに対応
    """
    ...
    def __init__(
        ...
        alpha_multiplier: float = 0.5, # アルファ調整係数
        # 動的期間パラメータ
        ...
        period_max: int = 20,                      # ピリオド最大値（インジケーター値低い時、偶数）
        # 平滑化モード設定
        ...
        Args:
            ...
            ... その他の動的期間パラメータ
            enable_indicator_adaptation: インジケーター動的適応を有効にするか（デフォルト: True）
            adaptation_indicator: 適応に使用するインジケーター（'hyper_er'、'hyper_adx'、'hyper_trend_index'、デフォルト: 'hyper_er'）
            hyper_er_period: HyperER計算期間（デフォルト: 14）
            hyper_er_midline_period: HyperERミッドライン期間（デフォルト: 100）
            hyper_adx_period: HyperADX計算期間（デフォルト: 14）
            hyper_adx_midline_period: HyperADXミッドライン期間（デフォルト: 100）
            hyper_trend_index_period: HyperTrendIndex計算期間（デフォルト: 55）
            hyper_trend_index_midline_period: HyperTrendIndexミッドライン期間（デフォルト: 100）
            fc_min: FC最小値（インジケーター値高い時）（デフォルト: 1.0）
            fc_max: FC最大値（インジケーター値低い時）（デフォルト: 8.0）
            sc_min: SC最小値（インジケーター値高い時）（デフォルト: 50.0）
            sc_max: SC最大値（インジケーター値低い時）（デフォルト: 250.0）
            period_min: ピリオド最小値（インジケーター値高い時、偶数）（デフォルト: 4）
            period_max: ピリオド最大値（インジケーター値低い時、偶数）（デフォルト: 24）
        ...
        indicator_name = f"HyperFRAMA(period={period_str}, src={src_type}, fc={fc}, sc={sc}, alpha_mult={alpha_multiplier}{adaptation_str})"
        ...
        if alpha_multiplier <= 0 or alpha_multiplier > 1:
            raise ValueError("alpha_multiplierは0より大きく1以下である必要があります")
    ...
    def calculate(self, data: Union[pd.DataFrame, np.ndarray]) -> HyperFRAMAResult:
        """
        ハイパーFRAMAを計算する
        ...
        Returns:
            HyperFRAMAResult: 通常FRAMA値、アルファ半分FRAMA値、フラクタル次元、アルファ値を含む結果
        """
        try:
            # キャッシュチェック
            ...
            if self.enable_indicator_adaptation and dynamic_fc is not None and dynamic_sc is not None:
                # インジケーター動的適応版を使用
                frama_values, half_frama_values, fractal_dim, alpha_values, half_alpha_values = calculate_hyper_frama_indicator_adaptation_core(
                    price, high, low, self.period, dynamic_fc, dynamic_sc, dynamic_periods, self.alpha_multiplier
                )
            elif self.period_mode == 'dynamic' and dynamic_periods is not None:
                # 動的期間対応版を使用
                frama_values, half_frama_values, fractal_dim, alpha_values, half_alpha_values = calculate_hyper_frama_dynamic_core(
                    price, high, low, self.period, self.fc, self.sc, dynamic_periods, self.alpha_multiplier
                )
            else:
                # 固定期間版を使用
                frama_values, half_frama_values, fractal_dim, alpha_values, half_alpha_values = calculate_hyper_frama_core(
                    price, high, low, self.period, self.fc, self.sc, self.alpha_multiplier
                )
            
            # 結果の保存
            result = HyperFRAMAResult(
                frama_values=frama_values.copy(),
                half_frama_values=half_frama_values.copy(),
                fractal_dimension=fractal_dim.copy(),
                alpha=alpha_values.copy(),
                half_alpha=half_alpha_values.copy(),
            ...
            self._values = frama_values  # 基底クラスの要件を満たすため（通常FRAMA値をメインとする）
            return result
            
        except Exception as e:
            error_msg = str(e)
            stack_trace = traceback.format_exc()
            self.logger.error(f"ハイパーFRAMA計算中にエラー: {error_msg}\n{stack_trace}")
            
            # エラー時は空の結果を返す
            error_result = HyperFRAMAResult(
                frama_values=np.array([]),
                half_frama_values=np.array([]),
    ...
    def get_half_frama_values(self) -> Optional[np.ndarray]:
        """
        アルファ半分FRAMA値を取得する
        
        Returns:
            np.ndarray: アルファ半分FRAMA値
        """
        if not self._result_cache:
            return None
        ...
        return result.half_frama_values.copy()
    ...
    def get_half_alpha(self) -> Optional[np.ndarray]:
        """
        アルファ半分値を取得する
