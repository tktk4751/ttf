...
def calculate_hody_dc_numba(
    ...
    """
    ホモダイン判別機を使ったドミナントサイクル検出のNumba実装
    ...
    for i in range(7, n):
        ...
        # Smooth the discriminator outputs
...
class EhlersHoDyDC(EhlersDominantCycle):
    """
    エーラーズのホモダイン判別機（Homodyne Discriminator）によるドミナントサイクル検出
    
    このアルゴリズムはヒルベルト変換を使用して価格データから直交成分（I/Q成分）を生成し、
    ホモダイン判別機を使用して周期を検出します。
    ...
    def __init__(
        ...
        """
        super().__init__(
            f"EhlersHoDyDC({cycle_part}, {max_cycle}, {min_cycle})",
            cycle_part,
            max_cycle,
            min_cycle,
            max_output,
    ...
    def calculate(self, data: Union[pd.DataFrame, np.ndarray]) -> np.ndarray:
        ...
        try:
            ...
            dom_cycle, raw_period, smooth_period = calculate_hody_dc_numba(
                price,
                self.cycle_part,
                self.max_cycle,
                self.min_cycle,
            ...
            self.logger.error(f"EhlersHoDyDC計算中にエラー: {error_msg}\n{stack_trace}")
            return np.array([]) 