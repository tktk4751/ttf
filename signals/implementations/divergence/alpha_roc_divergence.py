...
class AlphaROCDivergenceSignal(BaseSignal, IEntrySignal):
    """
    アルファROCダイバージェンスシグナル
    
    価格とアルファROCの間のダイバージェンスを検出し、エントリーシグナルを生成します。
    
    - 強気ダイバージェンス（ロングエントリー）：
      価格が安値を切り下げているのに対し、アルファROCが安値を切り上げている状態。
      上昇転換の可能性を示唆。
    
    - 弱気ダイバージェンス（ショートエントリー）：
      価格が高値を切り上げているのに対し、アルファROCが高値を切り下げている状態。
      下落転換の可能性を示唆。
    ...
    def generate(self, data: Union[pd.DataFrame, np.ndarray]) -> np.ndarray:
        ...
        try:
            ...
            # ダイバージェンスの検出（ROC値を使用）
            signals = self.divergence.generate(df, alpha_roc_result.roc)
            ...
            print(f"AlphaROCダイバージェンスシグナル生成中にエラー: {error_msg}\n{stack_trace}")
            # エラー時はゼロシグナルを返す
