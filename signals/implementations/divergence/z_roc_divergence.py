...
class ZROCDivergenceSignal(BaseSignal, IEntrySignal):
    """
    ZROCダイバージェンスシグナル
    
    価格とZROCの間のダイバージェンスを検出し、エントリーシグナルを生成します。
    
    - 強気ダイバージェンス（ロングエントリー）：
      価格が安値を切り下げているのに対し、ZROCが安値を切り上げている状態。
      上昇転換の可能性を示唆。
    
    - 弱気ダイバージェンス（ショートエントリー）：
      価格が高値を切り上げているのに対し、ZROCが高値を切り下げている状態。
      下落転換の可能性を示唆。
    ...
    def generate(self, data: Union[pd.DataFrame, np.ndarray]) -> np.ndarray:
        ...
        try:
            ...
            print(f"ZROCダイバージェンスシグナル生成中にエラー: {error_msg}\n{stack_trace}")
            # エラー時はゼロシグナルを返す
