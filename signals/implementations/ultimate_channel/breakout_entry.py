...
def calculate_ultimate_breakout_signals(
    close: np.ndarray, 
    upper: np.ndarray, 
    lower: np.ndarray, 
    lookback: int
    ...
    """
    アルティメットチャネルブレイクアウトシグナルを計算する（高性能版）
    
    Args:
        close: 終値の配列
        upper: 上部アルティメットチャネルの配列
        lower: 下部アルティメットチャネルの配列
...
class UltimateChannelBreakoutEntrySignal(BaseSignal, IEntrySignal):
    """
    アルティメットチャネルのブレイクアウトによるエントリーシグナル
    
    特徴:
    - John Ehlersのアルティメットチャネル技術
    - 動的適応乗数対応（UQATRDベース）
    - 線形補間による滑らかなチャネル調整
    - 高精度なブレイクアウト検出
    - 低遅延フィルタリング技術
    ...
    def __init__(
        ...
        Args:
            ...
            ultimate_channel_params: UltimateChannelに渡すパラメータ辞書（オプション）
        """
        # パラメータ設定
        ultimate_params = ultimate_channel_params or {}
        
        # チャネルパラメータを取得（シグナル名用）
        length = ultimate_params.get('length', 20.0)
        num_strs = ultimate_params.get('num_strs', 2.0)
        multiplier_mode = ultimate_params.get('multiplier_mode', 'fixed')
        src_type = ultimate_params.get('src_type', 'hlc3')
        
        super().__init__(
            f"UltimateChannelBreakoutEntrySignal(l={length}, n={num_strs}({multiplier_mode}), src={src_type}, lb={channel_lookback})"
        )
        
        # 基本パラメータの保存
        self._params = {
            'channel_lookback': channel_lookback,
            **ultimate_params  # その他のアルティメットチャネルパラメータ
        }
        
        # パラメータのハッシュ値を事前計算（_get_data_hash処理の高速化）
        self._params_hash = hash(tuple(sorted(self._params.items())))
            
        # アルティメットチャネルの初期化（すべてのパラメータを渡す）
        self.ultimate_channel = UltimateChannel(**ultimate_params)
    ...
    def generate(self, data: Union[pd.DataFrame, np.ndarray]) -> np.ndarray:
        ...
        try:
            ...
            if (data_hash == self._last_data_hash and 
                ...
                # アルティメットチャネルの計算
                result = self.ultimate_channel.calculate(data)
            ...
            # ブレイクアウトシグナルの計算（高速化版）
            signals = calculate_ultimate_breakout_signals(
                close,
                upper,
                lower,
            ...
            print(f"UltimateChannelBreakoutEntrySignal計算中にエラー: {str(e)}")
            import traceback
    ...
    def get_dynamic_multipliers(self, data: Union[pd.DataFrame, np.ndarray] = None) -> np.ndarray:
        ...
        if self._last_result is not None:
            return self._last_result.dynamic_multipliers
        else:
            return np.array([])
    ...
    def get_uqatrd_values(self, data: Union[pd.DataFrame, np.ndarray] = None) -> np.ndarray:
        ...
        if self._last_result is not None:
            return self._last_result.uqatrd_values
        else:
            return np.array([])
    ...
    def get_multiplier_mode(self, data: Union[pd.DataFrame, np.ndarray] = None) -> str:
        ...
        if self._last_result is not None:
            return self._last_result.multiplier_mode
        else:
            return 'fixed'
    ...
    def get_channel_statistics(self, data: Union[pd.DataFrame, np.ndarray] = None) -> Dict:
        ...
        if self._last_result is not None:
            return self.ultimate_channel.get_channel_statistics()
        else:
    ...
    def reset(self) -> None:
        ...
        """
        super().reset()
        if hasattr(self.ultimate_channel, 'reset'):
            self.ultimate_channel.reset()
