...
def generate_signals_numba(
    current_rsx: np.ndarray,
    prev_rsx: np.ndarray,
    high_levels: np.ndarray,
    low_levels: np.ndarray
) -> np.ndarray:
    """
    ZRSXトリガーシグナルを生成する（高速化版）
    
    Args:
        current_rsx: 現在のZRSX値の配列（0-100の範囲）
        prev_rsx: 1つ前のZRSX値の配列
        high_levels: 高値（買われすぎ）レベルの配列
        low_levels: 安値（売られすぎ）レベルの配列
    
    Returns:
        シグナルの配列 (1: 買いシグナル、-1: 売りシグナル、0: ニュートラル)
    """
    length = len(current_rsx)
    signals = np.zeros(length, dtype=np.int8)
    
    for i in prange(length):
        if np.isnan(current_rsx[i]) or np.isnan(prev_rsx[i]):
            continue
        
        # 買いシグナル: RSXが安値レベルを下から上に抜けた場合
        if prev_rsx[i] < low_levels[i] and current_rsx[i] >= low_levels[i]:
            signals[i] = 1
        
        # 売りシグナル: RSXが高値レベルを上から下に抜けた場合
        elif prev_rsx[i] > high_levels[i] and current_rsx[i] <= high_levels[i]:
            signals[i] = -1
    
    return signals


class ZRSXTriggerSignal(BaseSignal, IEntrySignal):
    """
    ZRSXを使用したトリガーシグナル
    
    特徴:
    - ZRSXが特定のレベルを抜けた時にのみシグナルを発生
    - サイクル効率比(ER)に基づく動的なしきい値レベル
    - ドミナントサイクルに基づく適応的な期間調整
    
    シグナル値:
    - 1: ZRSXが売られすぎレベルを下から上へ抜けた（買いシグナル）
    - -1: ZRSXが買われすぎレベルを上から下へ抜けた（売りシグナル）
    - 0: その他（シグナルなし）
    ...
    def __init__(
        ...
        super().__init__(
            f"ZRSXTrigger({max_dc_max_output}-{min_dc_min_output}, {min_high_level}-{max_low_level})",
            params
        )
    ...
    def generate(self, data: Union[pd.DataFrame, np.ndarray]) -> np.ndarray:
        ...
        try:
            ...
            # 前回のRSX値を計算（1つシフト）
            prev_values = np.roll(self._rsx_values, 1)
            prev_values[0] = self._rsx_values[0]  # 最初の要素は同じ値にする
            self._prev_rsx_values = prev_values
            
            # シグナルの生成（高速化版）
            signals = generate_signals_numba(self._rsx_values, prev_values, self._high_levels, self._low_levels)
            ...
            print(f"ZRSXTriggerSignal計算中にエラー: {error_msg}\n{stack_trace}")
    ...
    def get_levels(self) -> Tuple[np.ndarray, np.ndarray]:
        """
        適応的な高値/安値レベルを取得する
        
        Returns:
            (高値レベルの配列, 安値レベルの配列)のタプル
        """
        if self._high_levels is None or self._low_levels is None:
            empty = np.array([])
            return empty, empty
        return self._high_levels, self._low_levels
    ...
    def get_trigger_signals(self) -> np.ndarray:
        """
        トリガーシグナルを取得する
