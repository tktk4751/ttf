...
class HyperERChart:
    """
    Hyper ER（Hyper-Efficiency Ratio）を表示するローソク足チャートクラス
    
    - ローソク足と出来高
    - Hyper ER値とミッドライン（メインパネル下の別パネル）
    - トレンド信号（1=緑、-1=赤）（別パネル）
    """
    ...
    def calculate_indicators(self) -> None:
        ...
        # インジケーターの計算
        print("計算を実行します...")
        self.hyper_er_result = self.hyper_er.calculate(self.data)
        
        print("Hyper ERの計算完了")
        print(f"有効値数: {np.sum(~np.isnan(self.hyper_er_result.values))}/{len(self.hyper_er_result.values)}")
            
    def plot(self, 
            title: str = "Hyper ER チャート", 
        ...
        if self.hyper_er is None or self.hyper_er_result is None:
        ...
        hyper_er_values = self.hyper_er_result.values
        raw_er_values = self.hyper_er_result.raw_er
        midline_values = self.hyper_er_result.midline
        trend_signals = self.hyper_er_result.trend_signal
        ...
        if show_volume:
            # 出来高あり: パネル0=メイン、パネル1=出来高、パネル2=Hyper ER、パネル3=トレンド信号
            
            # Hyper ER値とミッドライン（パネル2）
            additional_plots.append(
                mpf.make_addplot(df['hyper_er'], panel=2, color='blue', width=2, 
                               ylabel='Hyper ER (0-1)', secondary_y=False, label='Hyper ER')
            )
            additional_plots.append(
                mpf.make_addplot(df['raw_er'], panel=2, color='lightblue', width=1, 
                               alpha=0.7, label='Raw ER')
            )
            additional_plots.append(
            ...
            additional_plots.append(
                mpf.make_addplot(df['trend_green'], panel=3, color='green', width=3, 
                               ylabel='Trend Signal', secondary_y=False, label='Efficient Trend', type='line')
            )
            additional_plots.append(
                mpf.make_addplot(df['trend_red'], panel=3, color='red', width=3, 
                               label='Inefficient (Range)', type='line')
            )
            
            panel_ratios = (4, 1, 2, 1)  # メイン:出来高:Hyper ER:トレンド信号
            
        else:
            # 出来高なし: パネル0=メイン、パネル1=Hyper ER、パネル2=トレンド信号
            
            # Hyper ER値とミッドライン（パネル1）
            additional_plots.append(
                mpf.make_addplot(df['hyper_er'], panel=1, color='blue', width=2, 
                               ylabel='Hyper ER (0-1)', secondary_y=False, label='Hyper ER')
            )
            additional_plots.append(
                mpf.make_addplot(df['raw_er'], panel=1, color='lightblue', width=1, 
                               alpha=0.7, label='Raw ER')
            )
            additional_plots.append(
            ...
            panel_ratios = (4, 2, 1)  # メイン:Hyper ER:トレンド信号
        ...
        valid_raw_er = df['raw_er'].dropna()
        valid_trend = df['trend_signal'].dropna()
        
        if len(valid_hyper_er) > 0:
            print(f"Hyper ER値範囲: {valid_hyper_er.min():.3f} - {valid_hyper_er.max():.3f}")
            print(f"Hyper ER平均: {valid_hyper_er.mean():.3f}")
            
        if len(valid_raw_er) > 0:
            print(f"Raw ER値範囲: {valid_raw_er.min():.3f} - {valid_raw_er.max():.3f}")
            print(f"Raw ER平均: {valid_raw_er.mean():.3f}")
            
        if len(valid_trend) > 0:
            total_signals = len(valid_trend)
            efficient_signals = len(valid_trend[valid_trend == 1])
            inefficient_signals = len(valid_trend[valid_trend == -1])
            
            print(f"総信号数: {total_signals}")
            print(f"効率的トレンド信号: {efficient_signals} ({efficient_signals/total_signals*100:.1f}%)")
            print(f"非効率的信号（レンジ）: {inefficient_signals} ({inefficient_signals/total_signals*100:.1f}%)")
...
def main():
    ...
    # コマンドライン引数を処理
    import argparse
    parser = argparse.ArgumentParser(description='Hyper ERチャートの描画')
