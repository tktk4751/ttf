...
class ZAdaptiveMAChart:
    """
    ZAdaptiveMA（Z適応型移動平均線）を表示するローソク足チャートクラス
    ...
    def calculate_indicators(self) -> None:
        ...
        z_ma_result = self.z_ma.calculate(self.data, external_er=er_result.values)
        
        print("ZAdaptiveMAと効率比の計算完了")
            
    def plot(self, 
            title: str = "ZAdaptiveMAチャート", 
        ...
        if self.z_ma is None or self.er is None:
        ...
        # ZAdaptiveMAと効率比の値を取得
        print("インジケーターデータを取得中...")
        z_ma_values = self.z_ma.get_values()
        er_values = self.er.get_values()
        trend_signals = self.z_ma.get_trend_signals()
        ...
        # トレンド方向に基づくZAdaptiveMAの色分け
        df['z_ma_uptrend'] = np.where(df['trend'] == 1, df['z_ma'], np.nan)
        df['z_ma_downtrend'] = np.where(df['trend'] == -1, df['z_ma'], np.nan)
        df['z_ma_range'] = np.where(df['trend'] == 0, df['z_ma'], np.nan)
        ...
        # ZAdaptiveMAのプロット設定
        main_plots.append(mpf.make_addplot(df['z_ma_uptrend'], color='green', width=2, label='ZAdaptiveMA (Up)'))
        main_plots.append(mpf.make_addplot(df['z_ma_downtrend'], color='red', width=2, label='ZAdaptiveMA (Down)'))
        main_plots.append(mpf.make_addplot(df['z_ma_range'], color='gray', width=2, label='ZAdaptiveMA (Range)'))
        ...
        # 凡例の追加
        axes[0].legend(['ZAdaptiveMA (Up)', 'ZAdaptiveMA (Down)', 'ZAdaptiveMA (Range)'], 
                      loc='upper left')
...
def main():
    ...
    # コマンドライン引数を処理
    import argparse
    parser = argparse.ArgumentParser(description='ZAdaptiveMAチャートの描画')
