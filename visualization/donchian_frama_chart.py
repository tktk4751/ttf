...
class DonchianFRAMAChart:
    """
    ドンチャンミッドラインとFRAMAを表示するローソク足チャートクラス
    
    - ローソク足と出来高
    - ドンチャンミッドライン、上部バンド、下部バンド（メインパネル上）
    - FRAMA（メインパネル上）
    - ドンチャントレンド信号（別パネル）
    - FRAMAフラクタル次元（別パネル）
    """
    ...
    def calculate_indicators(self) -> None:
        ...
        # ドンチャンミッドラインの計算
        print("ドンチャンミッドラインを計算中...")
        self.donchian = DonchianMidline(
            period=60,
            src_type='hlc3'
        )
        
        self.donchian_result = self.donchian.calculate(self.data)
        print(f"ドンチャンミッドライン計算完了: 有効値数 {np.sum(~np.isnan(self.donchian_result.values))}/{len(self.donchian_result.values)}")
    ...
    def plot(self, 
            title: str = "ドンチャンミッドライン & FRAMA チャート", 
        ...
        if self.donchian is None or self.frama is None:
        ...
        # ドンチャンミッドライン関連
        donchian_midline = self.donchian_result.values
        donchian_upper = self.donchian_result.upper_band
        donchian_lower = self.donchian_result.lower_band
        ...
        full_df = pd.DataFrame(
            ...
            data={
                'donchian_midline': donchian_midline,
        ...
        # 価格とドンチャンミッドラインの関係でトレンド信号を作成
        # NaN値をマスク
        valid_mask = ~(pd.isna(df['close']) | pd.isna(df['donchian_midline']))
        ...
        # 有効なデータのみで信号を計算
        if valid_mask.any():
            df.loc[valid_mask & (df['close'] > df['donchian_midline']), 'trend_up'] = 1
            df.loc[valid_mask & (df['close'] < df['donchian_midline']), 'trend_down'] = -1
            df.loc[valid_mask & (df['close'] == df['donchian_midline']), 'trend_neutral'] = 0
        ...
        if show_volume:
            ...
            # メインパネルにドンチャンバンドとFRAMA（パネル0）
            additional_plots.append(
                mpf.make_addplot(df['donchian_upper'], panel=0, color='red', width=1.5, 
                               alpha=0.7, label='Donchian Upper', linestyle='--')
            )
            additional_plots.append(
                mpf.make_addplot(df['donchian_midline'], panel=0, color='blue', width=2, 
                               label='Donchian Midline')
            )
            additional_plots.append(
                mpf.make_addplot(df['donchian_lower'], panel=0, color='green', width=1.5, 
                               alpha=0.7, label='Donchian Lower', linestyle='--')
            )
            additional_plots.append(
            ...
            additional_plots.append(
                mpf.make_addplot(df['trend_up'], panel=1, color='green', width=3, 
                               ylabel='Donchian Trend', secondary_y=False, label='Uptrend', type='line')
            )
            additional_plots.append(
        ...
        if show_volume:
            ...
            # フラクタル次元パネル（パネル1）に参照線
            axes[1].axhline(y=1.0, color='green', linestyle='--', alpha=0.5, label='Perfect Trend')
            axes[1].axhline(y=1.5, color='orange', linestyle='-', alpha=0.7, label='Threshold')
            axes[1].axhline(y=2.0, color='red', linestyle='--', alpha=0.5, label='Random Walk')
        ...
        # ドンチャンミッドライン統計
        valid_donchian = df['donchian_midline'].dropna()
        valid_trend_up = df['trend_up'].dropna()
        valid_trend_down = df['trend_down'].dropna()
        
        if len(valid_donchian) > 0:
            print(f"ドンチャンミッドライン範囲: {valid_donchian.min():.2f} - {valid_donchian.max():.2f}")
            print(f"ドンチャンミッドライン平均: {valid_donchian.mean():.2f}")
...
def main():
    ...
    # コマンドライン引数を処理
    import argparse
    parser = argparse.ArgumentParser(description='ドンチャンミッドライン & FRAMAチャートの描画')
